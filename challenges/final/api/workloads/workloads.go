...
func WorkloadsFilterResponse(c echo.Context) error {
	...
	if filter != "binary" && filter != "inverse" && filter != "filtertest" && filter != "zero" {
		return helpers.ReturnJSON(c, http.StatusConflict, "There is no filter with that name on the server!")
	}
